    ...
    const PICSUR_URL = process.env.PICSUR_URL || 'https://site-picsur.hjs9cn.easypanel.host';
    const PICSUR_API_KEY = process.env.PICSUR_API_KEY || 'IZh8ytuqAmEIK34SQlsaBFtVdvjBR47F';
    ...
    try {
        ...
        // O PICSUR espera o campo como "image" (conforme config do ShareX)
        form.append('image', new Blob([buf], { type: 'image/' + safeExt }), 'decoracao_' + Date.now() + '.' + safeExt);

        const resp = await fetch(PICSUR_URL + '/api/image/upload', {
            method: 'POST',
            // Cabeçalho correto do PICSUR legado
            headers: { 'Authorization': 'Api-Key ' + PICSUR_API_KEY },
            body: form
        });
        const json = await resp.json();
        if (!resp.ok || !json.data || !json.data.id) {
            return res.status(500).json({ success: false, message: 'Erro no PICSUR: ' + (json.data && json.data.message || resp.status) });
        }
        // Monta a URL final igual ao ShareX: /i/{id}.png
        const urlFinal = PICSUR_URL + '/i/' + json.data.id + '.' + safeExt;
        res.json({ success: true, url: urlFinal, delete_key: json.data.delete_key || null });
    } catch (err) {
    ...
    app.post('/api/pedidos', async (req, res) => {
        try {
            const { nome_cliente, telefone, endereco_rua, endereco_numero, endereco_bairro, endereco_cep, items, itens, valor_total, taxa_entrega, forma_pagamento, tipo_logistica, observacoes, data_entrega, hora_entrega, cliente_id, confirmar_whatsapp,imagens} = req.body;
            ...
            try {
                for (const img of imagens) {
                    ...
                    await pool.query(
                        `INSERT INTO pedido_imagens (pedido_id, url, tipo, observacao)
                         VALUES ($1,$2,$3,$4)`,
                        [pedido.id, img.url, img.tipo || 'decoracao', img.observacao || null]
                    );
                }
            } catch (errImg) {
                console.error('Erro ao gravar imagens do pedido:', errImg.message);
            }
        }
    ...
    app.post('/api/pedidos/consulta/:id', async (req, res) => {
        try {
            ...
            'SELECT * FROM pedido_imagens WHERE pedido_id = $1 ORDER BY id',
            [req.params.id]
        );
        res.json({ success: true, data: pedido, historico: historico.rows, itens: itensComComposicao, composicao: composicao.rows, imagens: imagens.rows });;
        } catch (err) {
    ...
    app.get('/api/pedidos/:id', authMiddleware, async (req, res) => {
        try {
            ...
            res.json({ success: true, data: result.rows[0], historico: historico.rows, itens: itensComComposicao, composicao: composicao.rows, imagens: imagens.rows });
        } catch (err) {
